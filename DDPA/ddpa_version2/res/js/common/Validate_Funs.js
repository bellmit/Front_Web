(function($){
	$(function(){
		...
		jQuery.validator.addMethod("isMinNumber",function(value,element){
				var allowZero = $(element).hasClass("allowZero");
			  var mtext= allowZero ? /^(([0-9]\d{0,9})|0)(\.\d{1,2})?$/ : /^(([1-9]\d{0,9})|0)(\.\d{1,2})?$/;
			  var values=value.replace(/(\s*)/g,"");
			  if(mtext.test(values)){
				 var tempvalue=parseInt(value);
				 if(tempvalue>=0 && allowZero){
					 return true;
				 }else if(tempvalue>0){
					 return true;
				 }else{
		...
		/*只能是邮箱或者手机*/
		jQuery.validator.addMethod("isPorE",function(value,element){
			  var tno = value.replace(/(\s*)/g,"");
			  if(tno==""){
				  return true;
			  }
