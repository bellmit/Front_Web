(function($){
	$(function(){
		...
		$("#pe_cance").click(function(){
			...
			window.location.href='manage_personinfo.html';
		});
		/*单选按钮事件监听*/
		...
		$("#popup_realaction").validate({
				...
				submitHandler:function(){
					$("#popup_realaction").find("input.popup_sure").attr("disabled",true);
					...
					$.ajax({
						...
						success : function(data){
							...
							if(data.success ==true){
								...
								$("#popup_realaction").find("input.popup_sure").removeAttr("disabled");
								$.unblockUI();
		...
		$("#popup_emailaction").validate({
				...
				submitHandler:function(){
					$("#popup_emailaction").find("input.popup_sure").attr("disabled",true);
					setTimeout(function(){
						$("#popup_emailaction").find("input.popup_sure").removeAttr("disabled");
					},60000);
					...
					$.ajax({
						...
						success : function(data){
							if (data.success) {
								$("#email_tips").html("认证邮件已经发送到您的邮箱：<a target='_self' href='"+data.emailUrl+"'>立即登录验证</a>");								
							}else{
...
KindEditor.ready(function(K){
	...
	var editor = K.editor({
		uploadJson : '../../upload/kindetor/imageUpload?userId='+userId+'&thumbSizes=150x100&imgSize=8',		
		imageSizeLimit : "8MB",
