		...
		if(public_tool.initMap.isrender){
			...
			$agent_list_wrap.delegate('span','click',function(e){
				...
				if(action==='delete'){
					...
					/*查看*/
					$.ajax({
							url:"http://10.0.5.222:8080/mall-agentbms-api/agent/view",
							method: 'POST',
							dataType: 'json',
							data:{
								"agentId":id,
					...
					/*绑定代理商请求数据*/
					$.when($.ajax({
						url:"http://10.0.5.222:8080/mall-agentbms-api/servicestation/notbound/list",
						method: 'POST',
						dataType: 'json',
						data:{
							"roleId":decodeURIComponent(logininfo.param.roleId),
						...
						url:"http://10.0.5.222:8080/mall-agentbms-api/servicestation/bound/list",
						method: 'POST',
						dataType: 'json',
						data:{
							"agentId":id,
			...
			$.each([$service_bindbtn,$service_unbindbtn], function () {
				...
				this.on('click', function () {
					...
					var $this=$(this),
						...
						config={
							url:"http://10.0.5.222:8080/mall-agentbms-api/servicestation/binding/operation",
							dataType:'JSON',
							method:'post',
							data:{
