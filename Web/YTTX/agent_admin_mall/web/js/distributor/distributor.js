	...
	$(function(){
		...
		if(public_tool.initMap.isrender){
			...
			var distributor_config={
				url:"http://10.0.5.222:8080/mall-agentbms-api/distributor/related",
				dataType:'JSON',
				method:'post',
			...
			/*统计数据加载配置*/
			var stats_config={
					url:"http://10.0.5.222:8080/mall-agentbms-api/distributor/profit/stats",
					dataType:'JSON',
					method:'post',
					data:{
						distributorId:'',
			...
			$distributor_wrap.delegate('span','click',function(e){
				...
				if(action==='select'){
					...
					if(subclass){
						...
						if(subitem===''){
							$.ajax({
								 url:"http://10.0.5.222:8080/mall-agentbms-api/distributor/lower",
								 method: 'POST',
								 dataType: 'json',
								 data:{
									 "distributorId":id,
