...
(function($){
	...
	$(function(){
		...
		if(public_tool.initMap.isrender){
			...
			/*数据加载*/
			var station_config={
				url:"http://10.0.5.222:8080/mall-agentbms-api/servicestations/related",
				dataType:'JSON',
				method:'post',
			...
			record_config={
				url:"http://10.0.5.222:8080/mall-agentbms-api/servicestations/invoice/search",
				dataType:'JSON',
				method:'post',
				dataSrc:function ( json ) {
					var code=parseInt(json.code,10);
			...
			$station_wrap.delegate('span','click',function(e){
				...
				if(action==='send'){
					...
					/*查看*/
					$.ajax({
							url:"http://10.0.5.222:8080/mall-agentbms-api/servicestation/view",
							method: 'POST',
							dataType: 'json',
							data:{
								"serviceStationId":id,
							...
							submitHandler: function(form){
								...
								if(issend){
									var checkdata=getCheckPlugin(send_checkconfig),
										config1={
											url:"http://10.0.5.222:8080/mall-agentbms-api/servicestation/invoice/add",
											dataType:'JSON',
											method:'post',
											data:{
												serviceStationId:id,
									...
									var config2={
										url:"http://10.0.5.222:8080/mall-agentbms-api/servicestation/repairorder/add",
										dataType:'JSON',
										method:'post',
										data:{
											serviceStationId:id,
