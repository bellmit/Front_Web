...
(/** @lends <global> */function( $ ) {
	...
	function _fnFeatureHtmlLength ( settings )
	{
		...
		div.children().append(
			settings.oLanguage.sLengthMenu.replace( '_菜单_', select[0].outerHTML )
		);
...
* */
	_api_register('ajax.config()',function(config){
		...
		return this.iterator( 'table', function ( settings ) {
			if ( $.isPlainObject( settings.ajax ) ) {
				settings.ajax = config;
			}
			else {
				settings.ajax.url = config;
			}
		});
	...
	_api_register( 'ajax.config().load()', function ( callback, resetPaging ) {
