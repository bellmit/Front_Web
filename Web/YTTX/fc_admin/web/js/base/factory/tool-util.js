...
(function ($) {
    ...
    angular.module('tool.util', []).factory('toolUtil', ['$http', '$q', '$httpParamSerializerJQLike', 'BASE_CONFIG', '$state', function ($http, $q, $httpParamSerializerJQLike, BASE_CONFIG, $state) {
        ...
        tools.loading = function (config) {
            ...
            if (type === 'show') {
                config.model.loadinghide = false;
            } else if (type === 'hide') {
                config.model.loadinghide = true;
            }
            /*清除延时指针*/
            if (config.delay) {
                clearTimeout(config.delay);
                config.delay = null;
            }
        };
        ...
        tools.resolveMainMenu = function (data, flag) {
            if (!data) {
                ...
                var menu_map = {
                        'yttx-oragnization': 'oragnization'/*机构管理,模块序列:10,true*/,
                        'yttx-order-manager': 'order'/*订单管理,模块序列:30,true*/,
                        'yttx-finance-manager': 'finance'/*财务管理*/,
                        'yttx-device-manager': 'equipment'/*设备管理*/,
                        'yttx-setting': 'setting'/*设置管理*/,
                        'yttx-invoice-manager': 'invoice'/*发货管理,模块序列:50,true*/,
                        'yttx-purchase-manager': 'purchase'/*采购管理,模块序列:70,true*/,
                        'yttx-warehouse-manager': 'warehouse'/*仓库管理,模块序列:90,true*/
                    },
                    list = [],
                    module = {},
                    menu = {},
