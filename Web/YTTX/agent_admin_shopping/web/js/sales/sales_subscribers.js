...
(function($){
	...
	$(function(){
		...
		if(public_tool.initMap.isrender){
			...
			/*数据加载*/
			var sales_config={
				url:"http://120.76.237.100:8081/mall-agentbms-api/subscribers/related",
				dataType:'JSON',
				method:'post',
			...
			/*查询服务站ID*/
			$.ajax({
				url:"http://120.76.237.100:8081/mall-agentbms-api/servicestations/list",
				dataType:'JSON',
				method:'post',
				data:{
					roleId:decodeURIComponent(logininfo.param.roleId),
			...
			$sales_list_wrap.delegate('span','click',function(e){
				...
				if(action==='update'){
					...
					$.ajax({
							url:"http://120.76.237.100:8081/mall-agentbms-api/subscriber/detail",
							method: 'POST',
							dataType: 'json',
							data:{
								"subscriberId":id,
					...
					if(subclass){
						...
						if(subitem===''){
							$.ajax({
								url:"http://120.76.237.100:8081/mall-agentbms-api/subscriber/acquirings",
								method: 'POST',
								dataType: 'json',
								data:{
									"subscriberId":id,
					...
					operate_item=$tr.addClass('item-lighten');
					$.ajax({
						url:"http://120.76.237.100:8081/mall-agentbms-api/subscriber/binding/user",
						method: 'POST',
						dataType: 'json',
						data:{
							"subscriberId":id,
				...
				if(formcache.form_opt_0){
					$.extend(true,form_opt,formcache.form_opt_0,{
						submitHandler: function(form){
							...
							if(isadd){
								var config={
									url:"http://120.76.237.100:8081/mall-agentbms-api/subscriber/add",
									dataType:'JSON',
									method:'post',
									data:{
										roleId:decodeURIComponent(logininfo.param.roleId),
								...
								var config={
									url:"http://120.76.237.100:8081/mall-agentbms-api/subscriber/update",
									dataType:'JSON',
									method:'post',
									data:{
										roleId:decodeURIComponent(logininfo.param.roleId),
