...
(function($){
	...
	$(function(){
		...
		if(public_tool.initMap.isrender){
			...
			$admin_username.on('focusout',function () {
				var $this=$(this),
					...
					if(username!==''){
						$.ajax({
							url:"http://120.24.226.70:8082/mall-agentbms-api/sysuser/check",
							dataType:'JSON',
							method:'post',
							data:{
								username:username,
						...
						$.extend(true,(function () {
							...
							submitHandler: function(form){
								...
								if(formtype==='addagent'){
									...
									config['url']="http://120.24.226.70:8082/mall-agentbms-api/agent/addupdate";
									config['data']=setdata;
								}
		...
		function requestGrade(fn){
			...
			/*查询上级运营商ID*/
			$.ajax({
				url:"http://120.24.226.70:8082/mall-agentbms-api/agent/role/check",
				dataType:'JSON',
				method:'post',
				data:{
					roleId:decodeURIComponent(logininfo.param.roleId),
		...
		function getSalesmanId() {
			$.ajax({
				url:"http://120.24.226.70:8082/mall-agentbms-api/salesmans/notused",
				dataType:'JSON',
				method:'post',
				data:{
					roleId:decodeURIComponent(logininfo.param.roleId),
